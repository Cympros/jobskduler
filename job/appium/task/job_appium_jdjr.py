...
class JobAppiumJDJRSignBase(AppiumBaseJob):
    '''
        用于进入京东金融的首页
    "'''

    def __init__(self):
        AppiumBaseJob.__init__(self, "com.jd.jrapp", "com.jd.jrapp.WelcomeActivity")
    ...
    def __deal_within_login(self):
        ...
        if self.query_ele_wrapper(self.get_query_str_within_xpath_only_text('京东APP一键登录'), click_mode="click",
            ...
            if self.query_ele_wrapper(self.get_query_str_within_xpath_only_text('京东一键登录'), click_mode="click",
                ...
                self.job_scheduler_failed("有'更多',但无'京东一键登录'")
                return False
        else:
            self.job_scheduler_failed("无更多入口")
            return False
        ...
        if self.query_ele_wrapper(
            ...
            if self.query_ele_wrapper(self.get_query_str_within_xpath_only_text('确认登录', view_type="android.view.View"),
                ...
                self.job_scheduler_failed("无'确认登录'")
                return False
        else:
            self.job_scheduler_failed("无'京东账号授权页面")
            return False
        return False
    ...
    def except_case_in_query_ele(self):
        ...
        if self.wait_activity(self.driver, ['.ver2.account.security.GestureLockActivity',
            ...
            self.job_scheduler_failed("请务必关闭手势，提高软件运行效率")
            return False
        ...
        # 检测到软件升级
            ...
            if self.query_ele_wrapper(self.get_query_str_by_viewid('com.jd.jrapp:id/cancel'), retry_count=0,
                ...
                self.job_scheduler_failed("检索到软件升级，但是没有检测到关闭按钮")
                return False
            return True
            ...
            if self.query_ele_wrapper(self.get_query_str_by_viewid('com.jd.jrapp:id/btn_ok'), retry_count=0,
                ...
                self.job_scheduler_failed('关闭单点登录风险提示框失败')
                return False
            return True
        return False
    ...
    def run_task(self):
        ...
        if not self.wait_activity(self.driver, search_fliter, retry_count=20):
            self.job_scheduler_failed("not in target activity")
            return False


class JobAppiumJDJRQuanyiCenter(JobAppiumJDJRSignBase):
    """
    ...
    def run_task(self):
        if JobAppiumJDJRSignBase.run_task(self) is False:
            return False
        if self.query_ele_wrapper(self.get_query_str_by_viewid('com.jd.jrapp:id/secondLayout'),
            ...
            self.job_scheduler_failed('找不到\"首页-支付\"按钮')
            return False
        # 滚动页面直至找到元素
        search_result = False
        ...
        if search_result is False:
            self.job_scheduler_failed("not found '权益中心'按钮")
            return False
        # 解析去赚京豆页面
        if self.query_ele_wrapper(self.get_query_str_by_desc('去赚京豆'), click_mode="click") is not None \
            ...
            self.job_scheduler_failed('not found \"去赚京豆\"')
            return False


class JobAppiumJDJRSign(JobAppiumJDJRSignBase):
    '''
        job:京东金融签到:首页-我-签到
        路径：首页-我-早起打卡
    '''
    ...
    def run_task(self):
        ...
        if self.query_ele_wrapper(self.get_query_str_by_viewid("com.jd.jrapp:id/home_title_portrait"),
                                  click_mode='click') is None:
            self.job_scheduler_failed("未找到左上角用户头像")
            return False
        # 个人中心-早起打卡的前一个tab
        if self.query_ele_wrapper(self.get_query_str_within_xpath_only_text('签到'), click_mode="click") is not None \
            ...
            if self.query_ele_wrapper(self.get_query_str_within_xpath_only_text("签到")) is None:
                self.job_scheduler_failed("未进入含'签到'标题字眼的界面")
                return False
        ...
        if self.query_only_point_within_text("^签到领钢*", is_auto_click=True) is not None \
            ...
            if self.query_ele_wrapper(self.get_query_str_within_xpath_only_text('今天已签到，明天再来吧~')) is not None \
                ...
                self.job_scheduler_failed(message="搜索成功字眼失败")
                return False

        else:
            self.job_scheduler_failed("未找到可触发'签到的按钮'")
            return False


class JobAppiumJDJRDailyClockOn(JobAppiumJDJRSignBase):
    '''
        Job:京东金融-每日打卡 --> 路径：首页-我-早起打卡
        - 需要开通免密支付
    '''

    def job_scheduler_failed(self, message, email_title=u'Error', is_page_source=True, is_scr_shot=True,
                             upload_files=[]):
        JobAppiumJDJRSignBase.job_scheduler_failed(self, message=message, email_title=email_title,
    ...
    def run_task(self):
        ...
        if self.query_ele_wrapper(self.get_query_str_by_viewid('com.jd.jrapp:id/home_title_portrait'),
                                  click_mode='click') is None:
            self.job_scheduler_failed("not found '个人中心'入口按钮")
            return False
        ...
        #     self.job_scheduler_failed("not found '我'的按钮")
        #     return False
        ...
        if self.query_ele_wrapper(self.get_query_str_within_xpath_only_text('早起打卡'), click_mode="click",
            ...
            if self.query_ele_wrapper("//android.view.View[@content-desc='打卡']", click_mode="click") is not None \
                ...
                if self.query_only_point_within_text('^继续坚持一天$', is_auto_click=True) is not None:
                    ...
                    # self.job_scheduler_failed('TODO:解析\"继续坚持一天\"之后的逻辑')
                    if self.query_ele_wrapper(self.get_query_str_within_xpath_only_text('立即支付'),
                        ...
                        if self.query_ele_wrapper("//android.view.View[@content-desc='支付成功 记得明早打卡哦']") is not None \
                            ...
                            self.job_scheduler_failed("没检测到'支付成功'标识")
                            return False
                    else:
                        self.job_scheduler_failed("没找到'立即支付'按钮")
                        return False
                else:
                    self.job_scheduler_failed('why not check 继续坚持一天')
                    return False
                ...
                if self.query_ele_wrapper(self.get_query_str_within_xpath_only_text('立即支付'),
                    ...
                    if self.query_ele_wrapper("//android.view.View[@content-desc='支付成功 记得明早打卡哦']") is not None:
                        ...
                        self.job_scheduler_failed("why not 解析到'支付成功'标识")
                        return False
                else:
                    self.job_scheduler_failed("why 底部没有出现立即支付的按钮")
                    return False
            else:
                self.job_scheduler_failed('进入早起打卡页面后解析元素失败')
                return False
        else:
            self.job_scheduler_failed('为什么找不到首页-早起打卡')
            return False
        return False
...
if __name__ == '__main__':
    tasks = ['JobAppiumJDJRDailyClockOn', 'JobAppiumJDJRQuanyiCenter', 'JobAppiumJDJRSign']
    while True:
        input_info = "------------------------执行任务列表-----------------------\n"
