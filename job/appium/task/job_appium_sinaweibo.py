...
class JobAppiumSinaWeiboDailyClockOnBase(JobAppiumSinaWeiboBase):
    ...
    def run_task(self):
        ...
        if self.query_only_point_within_text('^早起打卡$', is_auto_click=True, cutted_rect=[0, 0.25, 0, 0.5],
                ...
                and self.query_only_point_within_text('^早起打卡$', is_auto_click=True, cutted_rect=[0.25, 0.5, 0, 0.5],
                                                      retry_count=0) is None \
                and self.query_only_point_within_text('^早起打卡$', is_auto_click=True, cutted_rect=[0.5, 0.75, 0, 0.5],
                                                      retry_count=0) is None \
...
class JobAppiumSinaWeiboDailyClockOn(JobAppiumSinaWeiboDailyClockOnBase):
    ...
    def run_task(self):
        ...
        if self.query_ele_wrapper(self.get_query_str_within_xpath_only_text('打卡倒计时', view_type='android.widget.Button',
                ...
                or self.query_ele_wrapper(self.get_query_str_within_xpath_only_text('打卡倒计时', attribute='content-desc',
                                                                                    view_type='android.widget.Button',
                                                                                    is_force_match=False)) is not None:
            utils_logger.log("今日任务已完成")
            return True
        elif self.query_ele_wrapper(self.get_query_str_within_xpath_only_text('支付1元参与挑战', attribute='content-desc',
                                                                              view_type='android.widget.Button',
                                                                              is_force_match=False),
                                    click_mode="click") is not None:
            # 包含内容部分：'支付1元参与挑战 挑战成功奖金更多'
            self.query_ele_wrapper(self.get_query_str_within_xpath_only_text('立即支付', attribute='content-desc',
                                                                             view_type='android.widget.Button'),
                                   click_mode="click")
            # 输入支付密码
            if self.query_ele_wrapper(self.get_query_str_within_xpath_only_text('请输入支付密码')) is not None:
                if self.query_ele_wrapper("//android.widget.LinearLayout//android.widget.TextView[@text='5']",
                                          click_mode="click") is not None \
                        and self.query_ele_wrapper("//android.widget.LinearLayout//android.widget.TextView[@text='2']",
                                                   click_mode="click") is not None \
                        and self.query_ele_wrapper("//android.widget.LinearLayout//android.widget.TextView[@text='0']",
                                                   click_mode="click") is not None \
                        and self.query_ele_wrapper("//android.widget.LinearLayout//android.widget.TextView[@text='9']",
                                                   click_mode="click") is not None \
                        and self.query_ele_wrapper("//android.widget.LinearLayout//android.widget.TextView[@text='4']",
                                                   click_mode="click") is not None \
