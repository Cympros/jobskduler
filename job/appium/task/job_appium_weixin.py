...
class JobAppiumWeixinExit(AppiumBaseJob):
    ...
    def __init__(self):
        AppiumBaseJob.__init__(self, "com.tencent.mm", "com.tencent.mm.ui.LauncherUI")
                ...
                if self.query_ele_wrapper(self.get_query_str_within_xpath_only_text('退出'),
                    ...
                    if self.query_ele_wrapper(self.get_query_str_within_xpath_only_text("退出登录"),
                        ...
                        if self.query_ele_wrapper(
                            ...
                            self.job_scheduler_failed("退出失败")
                    else:
                        self.job_scheduler_failed("'退出登录'失败")
                else:
                    self.job_scheduler_failed("退出按钮 not found")
            else:
                self.job_scheduler_failed("未检测到'设置'入口")


class JobAppiumWeixinBase(AppiumBaseJob):
    """进入首页的基类"""
    ...
    def except_case_in_query_ele(self):
        ...
        if self.wait_activity(self.driver, '.plugin.account.ui.LoginPasswordUI', is_ignore_except_case=True,
            ...
            if weixin_pwd is None:
                self.job_scheduler_failed("未指定微信登录密钥")
                return False
            # 自登陆逻辑
            ...
            if ele_pwd_edt is not None:
                ...
                if self.query_ele_wrapper(
                    ...
                    self.job_scheduler_failed('找不到登录按钮', is_ignore_except_case=True)
            else:
                self.job_scheduler_failed('在登录界面却找不到密码输入框', is_ignore_except_case=True)
        # # 存储空间清理
        ...
        #         return True
        #     else:
        #         self.job_scheduler_failed("无法关闭'清除存储空间'的弹框")
            ...
            if self.query_ele_wrapper(
                ...
                self.job_scheduler_failed("无法关闭被踢下线的弹框")
        return False
    ...
    def run_task(self):
        ...
        if self.wait_activity(self.driver, '.ui.LauncherUI') is False:
            self.job_scheduler_failed('why not in main page of weixin')
            return False


class JobAppiumJimmieJDJingdonghuiyuan(JobAppiumWeixinBase):
    '微信-京东会员'

    def run_task(self):
        if JobAppiumWeixinBase.run_task(self) is False:
            return False
        if self.query_ele_wrapper(self.get_query_str_within_xpath_only_text('我'), click_mode="click") is None:
            self.job_scheduler_failed('not found \"我\"')
            return False
        if self.query_ele_wrapper(self.get_query_str_within_xpath_only_text('收藏'), click_mode="click") is None:
            self.job_scheduler_failed("\"收藏\"")
            return False
        if self.query_ele_wrapper(self.get_query_str_within_xpath_only_text('尊享京东会员新身份', is_force_match=False),
            ...
            self.job_scheduler_failed('尊享京东会员新身份')
            return False
        if self.query_only_point_within_text('^签$|^签到$', cutted_rect=[0.8, 1, 0.7, 1], is_auto_click=True) is not None:
            if self.query_point_size_within_text('^京豆优惠购$') > 0:
                ...
                self.job_scheduler_failed('not found 京豆优惠购')
        else:
            self.job_scheduler_failed('未找到右下角-签到按钮')
        return False


class JobAppiumJimmieJDCaiYunlianlianfan(JobAppiumWeixinBase):
    '微信-Jimmie-财运连连翻'
    ...
    def run_task(self):
        ...
        if self.query_ele_wrapper(self.get_query_str_within_xpath_only_text('财运连连翻'), click_mode="click") is None \
            ...
            self.job_scheduler_failed('财运连连翻')
            return False
        # TODO：以下仅做测试使用
        if self.query_only_point_within_text('占位符') is not None:
            utils_logger.log("---> 模拟使用占位符")
        else:
            self.job_scheduler_failed('财运连连翻 failed')
        return False


class JobAppiumJimmieJDJingdouleyuan(JobAppiumWeixinBase):
    '微信-Jimmie-京豆乐园'
    ...
    def run_task(self):
        ...
        if self.query_ele_wrapper(self.get_query_str_within_xpath_only_text('这里可以免费领京豆', is_force_match=False),
            ...
            self.job_scheduler_failed('这里可以免费领京豆')
            return False
        if self.query_only_point_within_text('^签到领京豆$', is_auto_click=True) is not None:
            ...
            if self.query_point_size_within_text('^知道了$|^京豆稍后会发放到您的账号$') > 0:
                ...
                self.job_scheduler_failed("解析'我知道了'失败")
                return False
        elif self.query_point_size_within_text(r'^已连续签到\d+天$|^今日邀请\d+位好友完成签到') > 0:
            utils_logger.log("今日已签到")
            return True
        else:
            self.job_scheduler_failed('无法领取京豆')
        return False


class JobAppiumJimmieJDPeiyuchang(JobAppiumWeixinBase):
    '微信-Jimmie-京豆培育舱'
    ...
    def run_task(self):
        ...
        if self.query_ele_wrapper(self.get_query_str_within_xpath_only_text('京豆培育舱'), click_mode="click") is None:
            self.job_scheduler_failed('未找到京豆培育舱')
            return False
        ...
        if is_view_layout_finished is False:
            ...
            self.job_scheduler_failed('在指定时限页面还未绘制完成')
            return False

        event_position = self.query_only_point_within_text('^(收获京豆|分享加速|培育京豆)$')
        if event_position is None:
            utils_logger.log("---> 未找到指定操作控件")
            self.job_scheduler_failed('未找到指定操作控件')
            return False
        # 构造比对子类
        base_src_img = job_root_path + "/job/appium/img/screen_shot_jingdong_jdpyc.png"
        outter_dir = env_job.get_out_dir()
        # 收获啦 图标
        ...
        for index in range(10):  # 随机滑动十次
            ...
            #         utils_logger.log("---> 选中指定item：",match_result_item
            #         if self.safe_tap_in_point(match_result_item['result']) is False:
            #             self.job_scheduler_failed("无法选中'培育京豆'item")
            #             return False
            ...
            if shouhuola_point_result is not None:
                for single_point in shouhuola_point_result:
                    if self.safe_tap_in_point(single_point['avaiable_point']) is False:
                        self.job_scheduler_failed("选择'收获啦'失败")
                        continue
                    utils_logger.log("---> 收获该item", event_position)
                    if self.safe_tap_in_point(event_position) is False:
                        self.job_scheduler_failed("收获京豆失败")
...
class JobAppiumShenqianxiaozhushou(JobAppiumWeixinBase):
    '''省钱小助手签到'''
    ...
    def run_task(self):
        ...
        if self.query_ele_wrapper(self.get_query_str_within_xpath_only_text('发送', view_type='android.widget.Button'),
            ...
            self.job_scheduler_failed('找不到发送的按钮')
            return False
        return False
        # if self.query_ele_wrapper(self.get_query_str_within_xpath_only_text('发送'),click_mode="click") is not None:
        #     self.job_scheduler_failed('搜索发送按钮')
        # else:
        #     self.job_scheduler_failed('搜索发送按钮失败')
        # return False


class JobAppiumZhaohangXinyongkaSign(JobAppiumWeixinBase):
    '招商信用卡签到领取积分'
    ...
    def run_task(self):
        ...
        if self.query_ele_wrapper("//android.view.View[@text='招商银行信用卡']", click_mode="click") is None \
            ...
            self.job_scheduler_failed('not found \"招商银行信用卡\"')
            return False
        # 截图1
        ...
        if self.query_ele_wrapper(self.get_query_str_within_xpath_only_text('领积分', is_force_match=False),
                ...
                and self.query_ele_wrapper(self.get_query_str_within_xpath_only_text('积分抽奖', is_force_match=False),
                                           click_mode="click") is None:
            self.job_scheduler_failed('not found \"领取任务\"')
            return False
        if self.query_ele_wrapper(self.get_query_str_within_xpath_only_text('签到领积分'), click_mode="click") is None \
                and self.query_ele_wrapper(self.get_query_str_within_xpath_only_text('签到领积分', is_force_match=False),
                                           click_mode="click") is None:
            self.job_scheduler_failed('not found \"签到领积分\"')
            return False
        ...
        # 等待10秒
        utils_logger.log("--->JobAppiumZhaohangXinyongkaSign sleep:", 10)
        time.sleep(10)
        return True


class JobAppiumWaziDashangchengBase(JobAppiumWeixinBase):
    '''
       微信-首页-袜子大商城-领取任务-京东免单/天猫免单
   '''

    def __init__(self, mode):
        JobAppiumWeixinBase.__init__(self)
        'mode:京东免单 or 天猫免单'
        self.mode = mode

    def notify_job_success(self):
        AppiumBaseJob.notify_job_success(self)
        utils_logger.log("--->更新上次check时间")
        if self.job_session is not None:
            conf_modify.put(job_tag=self.job_session, key="last_check_job_state_time",
    ...
    def run_task(self):
        ...
        if self.query_ele_wrapper("//android.view.View[@text='袜子大商城']", click_mode="click") is None \
            ...
            self.job_scheduler_failed('not found \"袜子大商城\"')
            return False
        ...
        if self.query_ele_wrapper(self.get_query_str_within_xpath_only_text(self.mode), click_mode="click") is None:
            self.job_scheduler_failed("not found \"" + self.mode + "\"")
            return False
        if self.query_point_size_within_text('^您在领取冷却中$') > 0:
            utils_logger.log("--->检测到任务冷却中")
            if self.job_session is not None:
                # TODO：检测到任务冷却中，因此强制把今日所有的该任务都清空(这种方式不够友好)
                conf_modify.put(job_tag=self.job_session, key="today_repeat_count_left", value=0)
            return True
            ...
            self.job_scheduler_failed(message="JobAppiumWaziDashangchengBase:" + self.mode,
                                      email_title=u'Maybe收到一笔新的袜子商城的任务')
            return True
        else:
            self.job_scheduler_failed('获取新任务失败')
            return False


class JobAppiumWaziDashangchengJingDong(JobAppiumWaziDashangchengBase):
    def __init__(self):
        JobAppiumWaziDashangchengBase.__init__(self, '京东免单')


class JobAppiumWaziDashangchengTaobao(JobAppiumWaziDashangchengBase):
    def __init__(self):
        JobAppiumWaziDashangchengBase.__init__(self, '天猫免单')


class JobAppiumJDJRQuanyiCenter(JobAppiumWeixinBase):
    """
        用于领取使用京东支付后的京豆奖励
    ...
    def except_case_in_query_ele(self):
        if JobAppiumWeixinBase.except_case_in_query_ele(self) is True:
            return True
        if self.query_ele_wrapper(self.get_query_str_within_xpath_only_text('微信登录'),
            ...
            if self.query_ele_wrapper(
                ...
                self.job_scheduler_failed("无法关闭京东授权弹窗")
            ...
            if self.query_ele_wrapper(self.get_query_str_within_xpath_only_text('登录'), click_mode='click',
                ...
                self.job_scheduler_failed("使用绑定的京东账号登陆失败")
        return False
    ...
    def run_task(self):
        ...
        if self.query_ele_wrapper(self.get_query_str_within_xpath_only_text('京东支付权益中心', is_force_match=False),
                                  click_mode='click') is None:
            self.job_scheduler_failed("找不到'京东支付权益中心'")
            return False
        # 解析去赚京豆页面
        for try_index in range(5):
            ...
            if self.query_ele_wrapper(self.get_query_str_by_desc('立即领取'), click_mode="click") is not None \
                ...
                if self.query_point_size_within_text('^恭喜你获得京豆$') > 0:
                    ...
                    if self.query_ele_wrapper(self.get_query_str_within_xpath_only_text('即将离开微信，打开其它应用')) is not None:
                        ...
                        if self.query_ele_wrapper(
                            ...
                            self.job_scheduler_failed("操作'允许'按钮失败")
                            return False
                    else:
                        self.job_scheduler_failed("没检测到'打开其他应用的弹框'")
                        return False
                else:
                    self.job_scheduler_failed('未检测到\"恭喜你获得京豆\"文案')
                    return False
                ...
                if self.query_ele_wrapper(self.get_query_str_by_desc('暂无京豆返利')) is not None \
                    ...
                    self.job_scheduler_failed('why not \"暂无京豆返利\"')
                    return False
            else:
                # 循环无法找到相应元素，发送错误提示并进行下次循环
                self.job_scheduler_failed("循环无法找到相应元素，发送错误提示")
        return False


if __name__ == '__main__':
    tasks = ['JobAppiumJDJRQuanyiCenter', 'JobAppiumJimmieJDCaiYunlianlianfan', 'JobAppiumJimmieJDJingdonghuiyuan',
             'JobAppiumJimmieJDJingdouleyuan',
             'JobAppiumJimmieJDPeiyuchang', 'JobAppiumShenqianxiaozhushou', 'JobAppiumWaziDashangchengBase',
             'JobAppiumWaziDashangchengJingDong',
             'JobAppiumWaziDashangchengTaobao', 'JobAppiumWeixinBase', 'JobAppiumWeixinExit',
             'JobAppiumZhaohangXinyongkaSign']
    while True:
        input_info = "------------------------执行任务列表-----------------------\n"
