...
def is_element_region_right(element, region_rect=None):
    ...
    if region_rect[0] <= int(ele_location['x']) <= region_rect[1] and region_rect[2] <= int(ele_location['y']) <= \
            region_rect[3]:
...
def touch_action(driver, target_device_name, is_down=True, tab_center=0.5, tab_interval=[0, 1], duration=300):
    '''
    :param driver: 
    ...
    :return: 
    '''
    utils_logger.log("--->[touch_action] tab_interval=", tab_interval, ",is_down=", is_down, ",tab_center", tab_center)
    ...
    if is_down:
        ...
        utils_logger.log("--->[touch_action] t_tab_center:", t_tab_center, ",t_from:", t_from, ",t_to:", t_to)
...
def start_appium_service(device, appium_port, access_appium_bp_port, retry_count=5, interval_time=5):
    '''启动appium服务,添加重试机制'''
    utils_logger.log("#start_appium_service# check_appium_service_state with retry_count:    ", retry_count)
    ...
    utils_logger.log("#start_appium_service# appium服务是否启动", "[" + appium_state_check_cmd + "]",
    ...
    if res_apm is not None:
        ...
        utils_logger.log("#start_appium_service# sleep:[" + str(interval_time) + "],[" + str(appium_start_cmd) + "]",
                         "[" + str(res) + "]", "[" + str(err) + "]")
...
def get_screen_shots(driver, target_device, file_path=None, retry_count=2):
    ...
    """
    if driver is None:
        utils_logger.log("--->[get_screen_shots] failed with no driver")
        return None
...
def get_pg_source(driver, retry_count=5):
    if driver is None:
        utils_logger.log('get_pg_source failed because driver is null')
        return None
...
def get_cur_act(driver, delay_time=0):
    if driver is None:
        utils_logger.log("get_cur_act failed because driver is null")
        return None
    if delay_time > 0:
...
def find_element_by_content_desc(driver, content_desc, retry_count=2, interval_time=0.1):
    '查询content-desc字段'
    utils_logger.log("find_element_by_content_desc[\"" + content_desc + "\"] with retry_count:", retry_count)
    element = None
    try:
        ...
        if element is not None:
            ...
            utils_logger.log("find_element_by_content_desc[" + content_desc + "] failed with no chance")
            return None
        else:
            utils_logger.log("--->find_element_by_content_desc sleep:", interval_time)
            time.sleep(interval_time)
            return find_element_by_content_desc(driver, content_desc, retry_count - 1, interval_time)
...
def wait_activity_with_status(driver, target, check_period=1, retry_count=10):
    ...
    def is_in_fliter(lists, search_fliter):
        ...
        search_status = False
        utils_logger.log("wait_activity_with_status.is_in_fliter<" + search_fliter + "> in <" + str(lists) + ">")
        for item in lists:
...
def get_driver_by_launch_app(application_id, launch_activity, device_name_to_connected, is_need_setting_input_manager,
    ...
    if application_id is None:
        utils_logger.log("#get_driver_by_launch_app# 应用包名不存在,当前展示应用的相关信息：[",
                         utils_android.get_top_focuse_activity(device_name_to_connected),
                         "]")
        return None
    ...
    if start_appium_service(device_name_to_connected, appium_port, access_appium_bp_port) is False:
        utils_logger.log("#get_driver_by_launch_app# start_appium_service失败[端口信息]：" + str(appium_port))
        return None
