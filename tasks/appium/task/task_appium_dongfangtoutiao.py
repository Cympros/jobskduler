...
class JobAppiumDongFangToutiaoBase(AppiumBaseJob):
    def __init__(self):
        AppiumBaseJob.__init__(self, 'com.songheng.eastnews', 'com.oa.eastfirst.activity.WelcomeActivity')

    def run_task(self):
        if AppiumBaseJob.run_task(self) is False:
            return False
        if self.wait_activity(self.driver, "com.songheng.eastfirst.common.view.activity.MainActivity") is False:
            self.job_scheduler_failed("未进入东方头条首页")
            return False


class JobAppiumDongFangtoutiaoCoreShiduanJiangli(JobAppiumDongFangToutiaoBase):
    def run_task(self):
        if JobAppiumDongFangToutiaoBase.run_task(self) is False:
            return False
        if self.query_ele_wrapper(self.get_query_str_within_xpath_only_text("领取"), click_mode="click") is not None:
...
class JobAppiumDongFangToutiaoYueDu(JobAppiumDongFangToutiaoBase):
    def run_task(self):
        if JobAppiumDongFangToutiaoBase.run_task(self) is False:
            return False
...
if __name__ == '__main__':
    tasks = ['JobAppiumDongFangToutiaoBase', 'JobAppiumDongFangtoutiaoCoreShiduanJiangli',
             'JobAppiumDongFangToutiaoYueDu']
    while True:
        input_info = "------------------------执行任务列表-----------------------\n"
